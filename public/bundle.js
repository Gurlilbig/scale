...
(function() {
  ...
  function initWebflowExtension() {
    ...
    const storedToken = retrieveWebflowToken();
    if (storedToken) {
      state.webflowToken = storedToken;
  ...
  function renderHomeView(container) {
    ...
    const isAuthenticated = !!state.webflowToken;
    
    main.innerHTML = `
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ${!isAuthenticated ? `
          <div class="bg-blue-100 p-6 rounded-lg shadow-md mb-8 text-center">
            <h2 class="text-2xl font-semibold mb-4">Connect Your Webflow Account</h2>
            <p class="mb-4 text-gray-700">To use all features of this extension, you need to connect your Webflow account.</p>
            <a 
              href="#" 
              class="button button-primary"
              id="connect-webflow-button"
            >
              Install Extension
            </a>
          </div>
        ` : `
          <div class="bg-green-100 p-6 rounded-lg shadow-md mb-8 text-center">
            <h2 class="text-2xl font-semibold mb-4">Webflow Account Connected</h2>
            <p class="mb-4 text-gray-700">Your Webflow account is successfully connected.</p>
            <button 
              class="button button-outline"
              id="logout-webflow-button"
            >
              Disconnect Account
            </button>
          </div>
        ...
        <div class="grid grid-cols-2 gap-8 max-w-4xl mx-auto">
          <div class="feature-card" id="resize-card">
            ...
            <button class="button button-primary" id="resize-button" ${!isAuthenticated ? 'disabled' : ''}>Get Started</button>
          </div>
          ...
          <div class="feature-card" id="crop-card">
            ...
            <button class="button button-primary" id="crop-button" ${!isAuthenticated ? 'disabled' : ''}>Get Started</button>
          </div>
        </div>
        ${!isAuthenticated ? `
          <div class="text-center text-gray-500 mt-6">
            <p>You need to connect your Webflow account to use these features.</p>
          </div>
        ` : ''}
      </div>
    `;
    ...
    setTimeout(() => {
      ...
      const logoutButton = document.getElementById('logout-webflow-button');
      if (logoutButton) {
      ...
      document.getElementById('resize-button')?.addEventListener('click', () => {
        if (isAuthenticated) {
          ...
          showPopupNotification({
            ...
            message: 'Please connect your Webflow account to use this feature.'
          });
        }
      });
      ...
      // Add OAuth connection event listener
      const connectButton = document.getElementById('connect-webflow-button');
      if (connectButton) {
  ...
  function storeWebflowToken(token) {
    if (token) {
      localStorage.setItem('webflowToken', token);
      state.webflowToken = token;
      console.log('Webflow token stored in localStorage');
      return true;
    }
    return false;
  }
  ...
  // Add a function to handle the OAuth window opening
  function openOAuthWindow() {
    // Replace with your actual OAuth URL
    const redirectUri = 'http://localhost:3000/oauth/callback';
    const oauthUrl = `https://webflow.com/oauth/authorize?response_type=code&client_id=dd8fa88280755d32abc22b7eaded0517c588f9467b1b574841f768ef8c4f44c7&scope=assets%3Aread%20assets%3Awrite&redirect_uri=${encodeURIComponent(redirectUri)}`;
    ...
    const messageHandler = function(event) {
      ...
      if (event.data && event.data.type === 'WEBFLOW_AUTH_SUCCESS') {
        ...
        if (storeWebflowToken(event.data.access_token)) {
          ...
          showPopupNotification({
            type: 'success',
            title: 'Connected!',
            message: 'Your Webflow account has been successfully connected. You can now use all features.'
          });
        }
      }
    };
