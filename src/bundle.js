...
(function() {
      ...
      setTimeout(() => {
        ...
        resizeButton.addEventListener('click', async () => {
          try {
            ...
            // Default to original dimensions if inputs aren't found or values are invalid
            ...
            // Step 2: Create an image element and load the blob
            const img = new Image();
            ...
            // Step 3: Create a canvas and resize the image
            ...
            if (keepAspectRatio) {
              ...
              // Just stretch to fill the entire canvas
              drawWidth = width;
              drawHeight = height;
            }
            ...
            console.log("Canvas converted to blob, size:", resizedBlob.size);
            ...
            const nameWithoutExt = originalFilename.includes('.') 
              ...
              : originalFilename;
            const resizedFilename = `${nameWithoutExt}-resized-${width}x${height}.jpg`;
            const resizedFile = new File([resizedBlob], resizedFilename, { type: 'image/jpeg' });
            
            // Step 6: Upload to Webflow
            console.log("Preparing to upload file:", resizedFilename);
            const formData = new FormData();
            formData.append('file', resizedFile);
            
            const uploadResponse = await fetch('http://localhost:3000/api/update-webflow-image', {
              method: 'POST',
              body: formData
            });
            ...
            successMessage.className = 'fixed top-4 right-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded shadow-md z-50';
            successMessage.innerHTML = `
              <div class="flex flex-col">
                <div class="flex items-center">
                  ...
                  <p>Image successfully resized and saved to Webflow assets!</p>
                </div>
                <p class="text-sm mt-1 ml-7 italic">NOTE: Please reload your Webflow page to see the resized image in Assets.</p>
              </div>
            `;
            ...
            // Redirect back to assets browser after successful upload
            state.currentView = 'assets-browser';
            renderApp(document.getElementById('root'));
            ...
            errorMessage.className = 'fixed top-4 right-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded shadow-md z-50';
            ...
            <div class="mb-6 text-center">
              <div id="crop-container" style="position: relative; display: inline-block; margin: 0 auto; overflow: hidden;">
                ...
                <div id="crop-overlay" style="position: absolute; top: ${state.crop.y}%; left: ${state.crop.x}%; width: ${state.crop.width}%; height: ${state.crop.height}%; border: 2px dashed #2563eb; background-color: rgba(59, 130, 246, 0.2); cursor: move; box-sizing: border-box;">
                  <div data-handle="tl" style="position: absolute; width: 12px; height: 12px; background-color: white; border: 2px solid #2563eb; top: -6px; left: -6px; cursor: nwse-resize; z-index: 2;"></div>
                  <div data-handle="tr" style="position: absolute; width: 12px; height: 12px; background-color: white; border: 2px solid #2563eb; top: -6px; right: -6px; cursor: nesw-resize; z-index: 2;"></div>
                  <div data-handle="bl" style="position: absolute; width: 12px; height: 12px; background-color: white; border: 2px solid #2563eb; bottom: -6px; left: -6px; cursor: nesw-resize; z-index: 2;"></div>
                  <div data-handle="br" style="position: absolute; width: 12px; height: 12px; background-color: white; border: 2px solid #2563eb; bottom: -6px; right: -6px; cursor: nwse-resize; z-index: 2;"></div>
                  <div data-handle="t" style="position: absolute; width: 12px; height: 12px; background-color: white; border: 2px solid #2563eb; top: -6px; left: 50%; transform: translateX(-50%); cursor: ns-resize; z-index: 2;"></div>
                  <div data-handle="r" style="position: absolute; width: 12px; height: 12px; background-color: white; border: 2px solid #2563eb; right: -6px; top: 50%; transform: translateY(-50%); cursor: ew-resize; z-index: 2;"></div>
                  <div data-handle="b" style="position: absolute; width: 12px; height: 12px; background-color: white; border: 2px solid #2563eb; bottom: -6px; left: 50%; transform: translateX(-50%); cursor: ns-resize; z-index: 2;"></div>
                  <div data-handle="l" style="position: absolute; width: 12px; height: 12px; background-color: white; border: 2px solid #2563eb; left: -6px; top: 50%; transform: translateY(-50%); cursor: ew-resize; z-index: 2;"></div>
                </div>
              </div>
            </div>
    ...
    async function uploadToWebflow(blob) {
      ...
      // Truncate name if it's too long to prevent Webflow filename length issues
      const truncatedName = nameWithoutExt.length > 30 
        ? nameWithoutExt.substring(0, 27) + '...' 
        : nameWithoutExt;
        
      const croppedFilename = `${truncatedName}-cropped-${cropWidth}x${cropHeight}.jpg`;
      ...
      formData.append('originalUrl', state.selectedAsset.url);
      formData.append('saveType', 'copy'); // Always save as copy
    ...
    async function handleFinishCropping() {
      try {
        ...
        // Truncate filename to avoid length issues
        const truncatedName = nameWithoutExt.length > 30 
          ? nameWithoutExt.substring(0, 27) + '...' 
          : nameWithoutExt;
        ...
        toastMessage.className = 'fixed top-4 right-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded shadow-md z-50';
        toastMessage.innerHTML = `
          <div class="flex items-center">
            <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
            </svg>
            <p>Image successfully cropped and uploaded to Webflow assets!</p>
          </div>
        `;
    ...
    function preloadImageDimensions(asset) {
      return new Promise((resolve, reject) => {
        ...
        const proxiedUrl = `http://localhost:3000/api/proxy-image?url=${encodedUrl}`;
    ...
    async function fetchWebflowAssets() {
      ...
      try {
        ...
        const response = await fetch('http://localhost:3000/api/webflow-assets');
